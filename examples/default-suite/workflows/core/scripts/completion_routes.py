#!/usr/bin/env python3
"""Route lookup shared by the launcher's completion commands.

`complete.py` (Tab), `completion.py` (popup items), and `route_separator.py`
(space) must agree on the candidate set, so they all import this module.

Nothing here hardcodes a workflow or View name: a package can be mounted under
any member id, so the routes and this workflow's own references are all resolved
from the suite the host is running.

Candidates come from `tflow --inspect --all`, which reports the suite's public
Views and their aliases. A View that declares no alias is a derived View (for
example `sys:output`) or the launcher itself, so it is not offered.

The result is cached for the lifetime of one script run: `complete.py` asks for
the candidate list and then for an exact match, which would otherwise spawn the
...
def owner_workflow():
    ...
    if _OWNER is not None:
        return _OWNER
    _OWNER = os.path.basename(os.environ.get("TFLOW_WORKFLOW_DIR") or "") or "core"
...
def own_view_ref():
    ...
    if workflow_dir:
        ...
        if isinstance(manifest, dict):
            entrypoint = str((manifest.get("workflow") or {}).get("entrypoint") or entrypoint)
