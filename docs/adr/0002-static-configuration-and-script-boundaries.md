...
[Static Values and Runtime Data](../reference/expressions.md) records the literal boundary and the explicit request/response alternatives to embedded expressions. The [Architecture Convergence plan](../explanation/architecture-convergence.md) records the implemented Router, task, execution, configuration ownership, and documentation boundaries.
