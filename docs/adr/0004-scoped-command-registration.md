...
- **Status**: Accepted
- **Date**: 2026-09-13 (Updated: 2026-09-16)
- **Scope**: The unified command layer as the sole input forwarding channel, discrete command registration across all engines, fallback input receiver contracts for unmapped raw input streams, scope lifecycles, input resolution, Footer active Enter resolution, command selection, navigation return values, and command execution requests.

## Decision Summary

`CommandRegistry` is the single source of truth for currently effective commands and the **exclusive input forwarding channel** for all keyboard interactions. All discrete keybindings (including engine-level controls such as Capture `copy`/`back` and Embedded `cancel`) must be registered in the command layer; registration acts as event subscription. Views and engines must not maintain private, unadvertised keymaps that intercept input behind the registry.

Input resolution, Footer rendering, and command selection use the same registry entries and revision; they do not maintain separate command lists or reconstruct commands from different contexts.

The registry has exactly three scopes: `Host`, `Engine`, and `View`. Scopes express priority only, without artificial metadata classifications. They do not express history, caller, origin, or the Router stack. The fixed priority order is:

```text
View > Engine > Host
```

For unmapped raw input streams (such as PTY streaming in Embedded views or typed character insertion), the active View explicitly implements the `FallbackInputReceiver` trait. Keystrokes that do not match any registered command in `CommandRegistry` are routed by the Session directly to `FallbackInputReceiver::on_unbound_key`. If the active View declares no fallback receiver, unbound input is safely ignored.

Four responsibilities are deliberately separate:
...
3. **Fallback input handling**: unmapped input flows exclusively through the explicit `FallbackInputReceiver` contract declared by the active View.
4. **Command execution**: the callback or its invoking caller completes execution according to the returned contract. Navigation carries results; it does not execute commands.
...
The command picker is the built-in `__commands:main` ordinary View with popup presentation. It reads its query, applies ordinary Picker selection, and returns an ordinary `ViewResult` through the Router. Parameter editing is the separate built-in `__form:main` native Form View. Neither built-in View accesses the registry, registers commands, or executes commands.
...
```text
Host    Fixed Host/session-level commands (e.g. Open Command Panel, Edit Query, user session bindings)
Engine  Discrete operational commands supplied by the active Engine (e.g. capture.copy, capture.back, embedded.cancel)
View    Specific business commands belonging to the current View ([workflows.<wf>.views.<v>.commands])
```
...
### Unified input forwarding, Hybrid Target execution, and FallbackInputReceiver contract

The `CommandRegistry` serves as the **exclusive input forwarding gateway** for all keyboard interactions. All discrete keystrokes must be registered in the registry; views and engines do not intercept or swallow keys via hidden internal keymaps.

Command entries support a **Hybrid Target Model** (`CommandHandler`):
- `CommandHandler::Action(Arc<dyn CommandAction>)`: Dispatches to an immutable closure or action object. Suited for Host session commands and TOML-declared business commands (scripts, navigation, calls).
- `CommandHandler::Event`: Dispatches as a typed event directly to the active View's exclusive mutable method `View::on_command(&mut self, id: &str, context: &ViewContext) -> Result<ViewDecision>`. Suited for stateful Engine-level commands (e.g., Picker navigation `select_previous`/`select_next`, `toggle_preview`, `clear_input`; Form field navigation `focus_next`/`focus_prev` with Tab/Down/BackTab/Up, and Form `cancel`/`exit`), allowing direct mutation of `&mut self` without `Arc<RwLock<...>>` synchronization overhead or borrow conflicts.

When a keyboard event arrives at the Session:

1. **Discrete command resolution**: The Session queries `CommandRegistry::resolve(key)`. If a matching entry is found across `View > Engine > Host`:
   - If `entry.handler` is `CommandHandler::Action(action)`, `action.execute()` is called.
   - If `entry.handler` is `CommandHandler::Event`, the Session obtains the active view from `Router::active_mut()` and invokes `active_instance.view.on_command(&entry.id, context)`.
   - The returned `ViewDecision` is processed through the Router with effects.
2. **Fallback input dispatch**: If the keystroke matches no registered command in `CommandRegistry`, the Session inspects the active View's `fallback_receiver()` declaration:
   - If the active View implements `FallbackInputReceiver`, the unmapped key and raw bytes are forwarded directly to `FallbackInputReceiver::on_unbound_key(key, raw, context)`. For example, `EmbeddedProtocolView` forwards unmapped byte streams to its child PTY, and `PickerProtocolView` / `FormView` forward unbound character typing and inline cursor navigation to their text buffers. All discrete semantic navigation keys (such as Tab/BackTab/Up/Down for form fields) are registered as discrete commands/events and never handled implicitly by fallback receivers.
   - If the active View returns `None` (i.e. it does not implement or declare a fallback receiver), the unmapped keystroke is safely dropped (`Ok(ViewDecision::Stay)`), preventing rogue unbound key leakage or accidental processing.

Keystrokes are never intercepted by private engine-side keymaps outside the registry.
...
The Footer renders the currently active `Enter` command resolved across `View > Engine > Host` precedence (rather than being restricted solely to the `View` scope). If an active Engine supplies an `Enter` binding (such as `capture.copy`), the Footer presents that binding unless an explicit View-level `Enter` command overrides it.
...
The Engine computes the complete command set required by the current View and current item for the Engine and View scopes. The active View exposes its Engine-scope commands via `View::engine_commands(&self, context: &ViewContext) -> Vec<CommandEntry>`. A View may provide inputs for its View scope, or the Engine may submit them on the View's behalf, but activation and replacement must have the same complete-set semantics.

Views that process raw, unbound input streams implement and expose `View::fallback_receiver(&mut self) -> Option<&mut dyn FallbackInputReceiver>`.
...
Current View/current item changes
  ...
  -> Chrome replaces its single snapshot
       ├─ Footer reads the snapshot (resolving active Enter across View > Engine > Host)
       └─ Host reads the snapshot to build the command-picker query

Key Input Arrives at Session
  -> Session queries CommandRegistry::resolve(key) (priority: View > Engine > Host)
       ├─ [Matched entry]
       │    ├─ Action(closure) -> Execute closure action.execute()
       │    └─ Event           -> Dispatch to active View on_command(&entry.id, context)
       │    -> Router processes resulting ViewDecision with effects
       └─ [Unmatched key]
            -> Session inspects active.view.fallback_receiver()
                 ├─ Some(receiver) -> receiver.on_unbound_key(key, raw, context) -> Router processes ViewDecision
                 └─ None -> Safe ignore (Ok(ViewDecision::Stay))
...
- Private engine keymaps or unadvertised key-consumption paths inside `View::event` that bypass `CommandRegistry`.
- Bypassing the scope priority hierarchy (`View > Engine > Host`) or introducing ad-hoc metadata classifications for command/input dispatch.
- Unbound input leakage or broadcast across views without an explicit `FallbackInputReceiver` trait implementation.
...
| Engine command registration | Active Engine (e.g. Capture `Copy`/`Back`, Embedded `Cancel`) registers discrete commands into `CommandScope::Engine`; Footer and resolve reflect them. |
| Fallback input receiver dispatch | Unmapped keys not in `CommandRegistry` are forwarded to active View's `FallbackInputReceiver`; unhandled keys without receiver are safely ignored. |
| Footer active Enter resolution | Footer dynamically displays the effective `Enter` command honoring `View > Engine > Host` precedence across View, Engine, and Host scopes. |
...
- Intercepting discrete keys inside engines or views via hidden private keymaps bypassing `CommandRegistry`;
- Adding non-scope metadata classifications or bypassing `View > Engine > Host` priority;
- Leaking or broadcasting unmapped raw input streams without explicit `FallbackInputReceiver` registration;
...
1. `CommandRegistry` is the single source of truth for currently effective commands and the exclusive input forwarding channel for all keyboard interactions.
...
4. The Host registers fixed Host commands, including `Ctrl-K`; the Engine and View register current dynamic commands.
5. All discrete keybindings (including engine-level controls) must be registered in the registry; engines and views do not consume keys through private internal keymaps.
6. Unmapped raw input streams flow exclusively through the active View's explicit `FallbackInputReceiver` trait implementation; views without a fallback receiver safely drop unmapped input.
7. The Chrome Footer resolves and renders the active `Enter` command according to `View > Engine > Host` precedence across all scopes.
8. Navigation caller identity comes from the Router stack's immediate View below the pushed View.
9. `NavigationRequest.return_contract` explicitly distinguishes `Normal` from `CommandSelection`.
10. The Router only pushes Views and propagates `ViewResult` values; it does not understand or dispatch commands.
11. The registry only maintains and resolves entries; it does not understand navigation.
12. Every effective replacement is atomic and publishes `CommandsChanged` with a revision.
13. Chrome maintains one current snapshot; Footer, input, and Host picker-query construction use that current data.
14. A Popup is an ordinary View plus presentation and uses ordinary query, Picker selection, Enter, and Return behavior.
15. A Popup never accesses the registry, registers commands, or executes commands.
16. The Host dispatches only a returned `CommandSelection` from the picker after revision validation; callbacks/invoking callers complete execution under the return contract.
17. Other Popups return `Normal` results to their own immediate callers through Router.
18. `owner`, `replace_owner`, legacy bindings/projection, `command_picker` special handling, caller/closed inference, and `enabled_query` are not part of the implementation.
