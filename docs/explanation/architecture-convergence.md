...
| 1. Foreground lifecycle | Implemented | Foreground effects use the production process-group API, terminal pgrp handoff, controlling-terminal child stdio where available, input discard, renderer invalidation, cancellation cleanup, stopped-child interruption policy, and configurable foreground command timeouts (`timeout_ms`). | Configurable foreground timeouts terminate the process group and reclaim the terminal; focused injection coverage verifies terminal restoration on fault. |
...
The `stage_5_scheduler_evidence` test entrypoint and `src/task/stage5_evidence.rs` collector provide an executable, reproducible baseline harness. In standard test runs, it verifies end-to-end task scheduling, correlation, and the 250 ms p95 SLO targets without adding ignored tests. Setting `STAGE5_FULL=1` or `STAGE5_EVIDENCE=1` executes the full 5 independent runs x 40 iterations benchmark workload and writes detailed `summary.md` and `raw.json` metrics artifacts under `target/stage5/scheduler-<timestamp>/`.
