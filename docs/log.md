...
- The setup wizard no longer discovers workflow packages by searching sibling directories. `workflow_source_dir()` now reads only the explicit `TFLOW_WORKFLOWS_BOOTSTRAP_DIR` debug override, and a git source will be added there later; `TFLOW_WORKFLOWS_DIR` and the `workflows/`/`tflow-workflows/` walk are gone. This also removes the silent fallback that installed whatever the cache happened to hold: with no source configured, `ensure_workflow_cache()` returns nothing and the install exits with a clear error instead of reinstalling a stale snapshot — which is how an old `[views.main.keymap] mode = "item"` survived a reinstall and then failed the new host with `unsupported key binding "mode"`. The startup-wizard integration tests were removed along with the discovery rules they pinned.

- Moved the View binding strategy out of the binding table: the `mode` entry inside `[views.<name>.keymap]` is now the View-level `keymap_mode` field (`"view"` default, `"item_merge"` for item-driven Views). `[views.<name>.keymap]` is a plain key-to-command map, so a key literally named `mode` is no longer shadowed, the field cannot collide with a binding, and the normalization pass no longer rewrites a `mode` entry; `--inspect` reports the choice as `keymap_mode`. The dead `Command.scope` field (parsed but never read; the palette's scope badge comes from the command registry) is gone.
...
- The wizard no longer needs environment setup to find the workflows: `TFLOW_WORKFLOWS_BOOTSTRAP_DIR` (or `TFLOW_WORKFLOWS_DIR`) wins, and a bundled `workflows/` beside the distribution or a sibling `tflow-workflows/` checkout is discovered by walking up from `TFLOW_WORKFLOW_DIR`. It reports the resolved source and cache in its summary, and the suite target is read from the core package's `[workflow].entrypoint` rather than assuming `core:main`. (Removed on 2026-09-22: the wizard now reads only the debug override and never searches the tree.)
...
- Added `tests/wizard.rs` (removed on 2026-09-22): it drives the wizard's embedded scripts against a private copy of the workflow source and pins the two regressions (a cache that lags its source, a merge copy that keeps deleted files), plus the generated manifest, settings and the shipped wizard's own `--check`.
