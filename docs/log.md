...
- Added a route-completion how-to: the aggregate View base keymap, the `navigate`-only completion command, the popup View, the shared `--inspect --all` candidate lookup, the Space separator, and the optional `left_prefix` prompt. It documents the behaviour that disappeared from the engine when route entry moved into the workflow.
