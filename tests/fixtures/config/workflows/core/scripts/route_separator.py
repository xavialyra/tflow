...
Space is bound as a command, so the engine does not insert it. If the current
input names a known route the command jumps to it and consumes the input.
Otherwise it re-mounts this View with a literal space appended, carrying the
current parameters and the selected item (`__focus`) so filtering and selection
...
match = completion_routes.find_route(selector) if selector else None
