use super::model::{RawTheme, ThemeRef};
use super::*;
...
fn backdrop_style_inherits_and_accepts_explicit_overrides() {
    ...
    assert_eq!(theme.chrome.backdrop.fg, Some(Color::DarkGray));
    assert_eq!(theme.chrome.backdrop.bg, None);
...
fn dynamic_scheme_names_and_merged_references() {
    ...
    assert_eq!(theme.capture.text.fg, theme.scheme.get("muted").copied());
...
fn invalid_colors_and_unknown_references_report_source_and_field() {
    for value in [
        ...
        for field in [
            ...
            let raw: RawTheme = toml::from_str(&format!("[{field}]\n{key} = {value:?}\n")).unwrap();
...
fn explicit_refs_and_style_fields_keep_their_names_in_errors() {
    let ref_error = toml::from_str::<ThemeRef>("source = \"mystery\"\nname = \"x\"\n").unwrap_err();
