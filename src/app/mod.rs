...
mod tests {
    ...
    #[test]
    fn terminal_restore_failure_is_propagated_as_a_fatal_host_error() {
        let mut master = -1;
        let mut slave = -1;
        assert_eq!(
            unsafe {
        ...
        .unwrap();
        terminal.fail_next_foreground_resume();

        let error = match run_foreground_effect(
            &mut terminal,
            PreparedProcess {
            ...
            Err(error) => error,
            Ok(_) => panic!("injected terminal resume failure should fail the host"),
        };
        assert!(error.downcast_ref::<FatalHostError>().is_some());
        assert!(
            error
                .to_string()
                .contains("could not restore launcher terminal")
        );

        let mut reported = false;
        let propagated = report_or_propagate_protocol_error(error, |_| reported = true);
        assert!(
            !reported,
            "fatal host errors must not become TUI diagnostics"
        );
        assert!(
            propagated
                .expect_err("fatal host errors must stop dispatch")
                .downcast_ref::<FatalHostError>()
                .is_some()
        );
