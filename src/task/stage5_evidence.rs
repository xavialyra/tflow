...
const FULL_ITERATIONS: usize = 40;
...
pub(super) fn run() {
    let full_mode =
        std::env::var("STAGE5_FULL").is_ok() || std::env::var("STAGE5_EVIDENCE").is_ok();
    let runs_count = if full_mode { FULL_RUNS } else { DEFAULT_RUNS };
    let iterations_count = if full_mode {
        FULL_ITERATIONS
    } else {
        DEFAULT_ITERATIONS
    };
    ...
    for run_index in 0..runs_count {
        ...
        let mut measurements = metric_map(iterations_count);
        ...
        let snapshot = runtime.metrics_snapshot();
        validate_run(&runtime, &snapshot, iterations_count);
        for name in METRICS {
            assert_eq!(measurements[name].len(), iterations_count);
            pooled
    ...
    let raw = json!({
        "schema": "tflow.stage5.scheduler-evidence.v1",
        "collector": {
        ...
        "scenario": {
            "runs": runs_count, "iterations_per_run": iterations_count,
    ...
    // Verify 250ms P95 SLO on all metrics
    let threshold_ns = 250_000_000_u64;
    for name in METRICS {
        ...
        assert!(
            p95 <= threshold_ns,
            "Metric {name} exceeded 250ms P95 SLO: {p95} ns"
        );
    }
    ...
    if full_mode {
        ...
        fs::write(
            ...
            markdown(&raw, &summaries, runs_count, iterations_count),
        )
        .unwrap();
        println!(
            "Stage 5 scheduler evidence generated: {}",
            directory.display()
        );
    } else {
        // Clean up temporary ready files created in test run
...
fn validate_run(runtime: &TaskRuntime, metrics: &TaskRuntimeMetricsSnapshot, iterations: usize) {
...
fn terminal_json(sample: &TaskTerminalSample) -> Value {
    json!({
        "tags": {"engine": sample.tags.engine, "task_class": sample.tags.task_class},
        "lane": sample.lane,
        "submitted_at_ns": ns(sample.submitted_at.0).to_string(),
        "started_at_ns": sample.started_at.map(|time| ns(time.0).to_string()),
        ...
        "terminal_at_ns": ns(sample.terminal_at.0).to_string(),
...
fn metrics_json(metrics: &TaskRuntimeMetricsSnapshot) -> Value {
    json!({
        "now_ns": ns(metrics.now.0).to_string(),
        ...
        "panicked_total": metrics.panicked_total,
        "cancelled_total": metrics.cancelled_total,
        "submission_after_shutdown_total": metrics.submission_after_shutdown_total,
        "worker_started_total": metrics.worker_started_total,
        "worker_joined_total": metrics.worker_joined_total,
        "recent_terminal_count": metrics.recent_terminal.len()
    })
}

fn metric_map(iterations: usize) -> BTreeMap<String, Vec<u64>> {
    METRICS
        .into_iter()
...
fn stats(values: &[u64]) -> Value {
    json!({
        "count": values.len(),
        "p50_ns": rank(values, 50).to_string(),
        "p95_ns": rank(values, 95).to_string(),
        "max_ns": values.iter().max().unwrap().to_string()
    })
}
...
fn summaries(
    ...
    METRICS
        ...
        .map(|name| {
            let p50 = by_run
                .iter()
                .map(|run| rank(&run[name], 50))
                .collect::<Vec<_>>();
            let p95 = by_run
                .iter()
                .map(|run| rank(&run[name], 95))
                .collect::<Vec<_>>();
            let max = by_run
                .iter()
                .map(|run| *run[name].iter().max().unwrap())
                .collect::<Vec<_>>();
            (
                name,
                json!({
                    "pooled": stats(&pooled[name]),
                    "median_of_run_p50_ns": rank(&p50, 50).to_string(),
                    "median_of_run_p95_ns": rank(&p95, 50).to_string(),
                    "median_of_run_max_ns": rank(&max, 50).to_string()
                }),
            )
        })
        .collect()
...
fn collector_invocation(profile: &str) -> &'static str {
    match profile {
        "release" => "cargo test --release --lib stage_5_scheduler_evidence -- --exact --nocapture",
        _ => "cargo test --lib stage_5_scheduler_evidence -- --exact --nocapture",
    }
}
...
fn environment() -> Value {
    json!({
        ...
        "kernel": command_stdout("uname", &["-srmo"]),
        "arch": std::env::consts::ARCH,
        "cpu_count": std::thread::available_parallelism().ok().map(|n| n.get()),
        "git_commit": command_stdout("git", &["rev-parse", "HEAD"]),
        "git_dirty": command_stdout("git", &["status", "--porcelain"]).is_some_and(|s| !s.is_empty()),
        "cargo_lock_sha256": command_stdout("sha256sum", &["Cargo.lock"]).and_then(|s| s.split_whitespace().next().map(str::to_string))
    })
}
